...
function buildCandidates(slug: string, position: Position, brand?: BrandLike | null) {
  const urls: string[] = [];
  const v = brand?.banners?.version
    ? `?v=${encodeURIComponent(String(brand.banners.version))}`
    : '';
...
export default function EventBanner({
  ...
  const candidates = useMemo(
    () => buildCandidates(slug, position, brand),
    [slug, position, brand]
  );
  ...
  const href =
    clickableHref || (brand?.banners?.[position]?.href ?? '');

  return (
    <div
      className={`w-full flex items-center justify-center overflow-hidden ${className}`}
      style={{
        padding: position === 'header' ? '12px 0' : '16px 0',
      }}
    >
      {href ? (
