...
import type { AttendeeMeta } from './meta';
...
/** HTML that mirrors the badge preview (QR inline via cid:ticket-qr, button → ticket page) */
export function buildBadgeHTML(args: {
  ...
  const { brand, event, token } = args;
  const primary = brand.primary ?? '#111827';
  const secondary = brand.secondary ?? '#9aa3af';
  const button = brand.button ?? '#2e5fff';

  const fullName =
    [args.meta?.firstName, args.meta?.lastName].filter(Boolean).join(' ').trim() || 'FULL NAME';
  const jobTitle = args.meta?.jobTitle || 'JOB TITLE';
  const company = (args.meta?.companyName ?? (args.meta as any)?.company) || 'COMPANY NAME';
  ...
  const ticketUrl = `${(args.appUrl ?? process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3000')
