...
export async function GET(req: Request) {
  try {
    ...
    const label   = url.searchParams.get('label')   ?? 'VISITOR';

    // Optional event lines (purely decorative; PNG route is offline-friendly)
    const eventLine1 = url.searchParams.get('eventTitle') ?? '';
    const eventLine2 = url.searchParams.get('eventTime')  ?? ''; // e.g., "Aug 22, 10:00 • Hall A"
    ...
    // Sizing / quality (defaults are print friendly)
    ...
    const height  = Math.round(width * 1.45); // same ratio you used

    // Generate QR as SVG (paths), no margins
    ...
    // Strip XML header and ensure crisp edges, then set width/height attrs (nested <svg> works with sharp)
    const qrInner = qrSvgRaw
      .replace(/<\?xml[^>]*>/, '')
      .replace('<svg', `<svg width="${qrSize}" height="${qrSize}" shape-rendering="crispEdges"`);

    // Optional event block height
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <!-- Smooth diagonal sheen; animations won't run in PNG, so we render a bold static band -->
  ...
  <g font-family="Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif" text-anchor="middle">
    <text x="${width/2}" y="${headerH + pad + qrSize + Math.round(pad*1.4)}"
          fill="#111827" font-weight="900" font-size="${Math.round(width*0.06)}">${esc(name)}</text>
    <text x="${width/2}" y="${headerH + pad + qrSize + Math.round(pad*2.6)}"
          fill="#1E61FF" font-weight="800" font-size="${Math.round(width*0.042)}">${esc(title)}</text>
    <text x="${width/2}" y="${headerH + pad + qrSize + Math.round(pad*3.8)}"
          fill="#334155" font-weight="700" font-size="${Math.round(width*0.036)}">${esc(company.toUpperCase())}</text>
  </g>
  ...
  <!-- Diagonal sheen band (static for PNG) -->
  <g style="mix-blend-mode:soft-light">
    ...
    // Render to PNG via sharp (density controls SVG rasterization quality)
