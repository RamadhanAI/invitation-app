  ...
  return (
    ...
    >
      ...
      <form method="POST" action="/api/auth/login" className="relative w-full max-w-md">
        <div
          className="mx-auto mb-4 h-[2px] w-24 rounded-full"
          style={{
            background: `linear-gradient(90deg, transparent, ${goldLux}, transparent)`,
            boxShadow: goldGlow,
          }}
        />

        <div
          className="p-6 border rounded-2xl md:p-8"
          style={{
        ...
        >
          ...
          <p className="mt-1 text-sm" style={{ color: 'rgba(255,255,255,.68)' }}>
            Super admin uses <span style={{ color: goldLux }}>.env</span> credentials. Tenant staff use their
            <span style={{ color: goldLux }}> email + password</span>.
          </p>

          {err === '1' && (
            <div
              className="px-3 py-2 mt-4 text-sm rounded-lg"
              style={{
                color: '#fff',
          ...
          {err === '2' && (
            <div
              ...
              style={{
                color: '#fff',
                background: 'linear-gradient(180deg, rgba(234,179,8,.22), rgba(234,179,8,.10))',
                border: '1px solid rgba(234,179,8,.35)',
              }}
            >
              Your tenant is not approved yet (or has been suspended). If you just signed up, hang tight — or
              <a href="/request-demo" className="underline ml-1" style={{ color: goldLux }}>
                request access
              </a>
              .
            </div>
          ...
          <label className="block mt-6">
            <div className="mb-1 text-sm" style={{ color: 'rgba(255,255,255,.8)' }}>
              Email or Username
            </div>
            <input
              name="identifier"
          ...
          <button
            type="submit"
            className="w-full mt-6 font-semibold tracking-wide rounded-xl"
            style={{
              minHeight: 46,
              color: '#0b0d10',
          ...
          <div className="flex items-center justify-between mt-4 text-sm">
            <a href="/" className="underline" style={{ color: goldLux }}>
              Back to site
            </a>
            <a href="/api/auth/logout" className="underline" style={{ color: 'rgba(255,255,255,.6)' }}>
              Reset session
            </a>
          </div>
        </div>
